...
<jsp:include page="../admin_top.jsp"/>
	<!-- s: content -->
    <section id="admin_dessertlist" class="content menuList">
        <div class="inner_wrap">
            ...
            <div class="search_box">
	            <form name="f" action="admin_dessertlist" method="post">
	                <label>메뉴명
            ...
            <div class="list_box">
                <div class="btn_box">
                    <button type="button" onclick="window.location='admin_adddessert'">메뉴등록</button>
                </div>
                ...
                <table class="search_list s_table">
                    ...
                    <tbody>
                    	<c:if test="${empty dessertList}">
                    	<tr>
                            <td colspan="5">등록된 음료가 없습니다.</td>
                        </tr>
                    	</c:if>
                    	<c:forEach var="dto" items="${dessertList}">
                        <tr>
                            <td>${fn:substring(dto.menuCode,1,3)}</td>
                            <td>${dto.menuCode}</td>
                            <td><a href="admin_editdessert?menuCode=${dto.menuCode}">${dto.menuName}</a></td>
                ...
                <!-- 페이징 -->
                <c:if test="${not empty dessertList}">
                <div class="pagination">
		            <c:if test="${startPage > pageBlock}"> 
			        	<a class="page_btn prev_btn" href="admin_dessertlist?pageNum=${startPage-3}&menuName=${param.menuName}&menuStatus=${param.menuStatus}"><img src="../../images/icons/arrow.png"></a>
			    	</c:if>
				    ...
				    <c:forEach var="i" begin="${startPage}" end="${endPage}">
					    ...
					    <a href="admin_dessertlist?pageNum=${i}&menuName=${param.menuName}&menuStatus=${param.menuStatus}" class="${activeClass} page_num">${i}</a>
					</c:forEach>
				    
				    <c:if test="${pageCount > endPage}">
				        <a class="page_btn next_btn" href="admin_dessertlist?pageNum=${startPage+3}&menuName=${param.menuName}&menuStatus=${param.menuStatus}"><img src="../../images/icons/arrow.png"></a>
				    </c:if>
	            </div>
                </c:if>
            </div>
